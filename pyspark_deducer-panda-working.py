...
def main(input_file, custom_cols):
    ...
    def pseudonymize(unique_names):
        ...
        assert len(unique_names) == len(name_map.items(
        )), "Pseudonymization function safeguard: unique_names (input) and name_map (output) don't have same length"
        return name_map
    ...
    # psdf.coalesce(1).write.mode("overwrite").option("header", "true").csv(output_file)



    # output_dir = os.path.join("data", "output")
    # base_filename = os.path.splitext(os.path.basename(input_file))[0]
    # output_path = os.path.join(output_dir, base_filename + "_processed")
